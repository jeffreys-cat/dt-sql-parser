...
statementBase
    ...
    | LOAD LABEL lableName=identifier LEFT_PAREN dataDescs+=dataDesc (COMMA dataDescs+=dataDesc)* RIGHT_PAREN (
        withRemoteStorageSystem
    )? (PROPERTIES LEFT_PAREN properties=propertyItemList RIGHT_PAREN)? (commentSpec)? # load
...
unsupportedStatement
    ...
    | SET NAMES (charsetName=identifierOrText | DEFAULT) (
        COLLATE collateName=identifierOrText
        | DEFAULT
    )? # setCollate
    | SET PASSWORD (FOR userIdentify)? EQ (
        STRING_LITERAL
        | (PASSWORD LEFT_PAREN STRING_LITERAL RIGHT_PAREN)
    ) # setPassword
    | SET LDAP_ADMIN_PASSWORD EQ (
        STRING_LITERAL
        | (PASSWORD LEFT_PAREN STRING_LITERAL RIGHT_PAREN)
...
dataDesc
    ...
    )* RIGHT_PAREN INTO TABLE tableName=multipartIdentifier (PARTITION partition=identifierList)? (
    ...
    | ((WITH)? mergeType)? DATA FROM TABLE tableName=multipartIdentifier INTO TABLE tableName=multipartIdentifier (
...
explain
    : (EXPLAIN planType? | DESC | DESCRIBE) level=(VERBOSE | TREE | GRAPH | PLAN)? PROCESS?
    ;
...
mysqlDataDesc
    : DATA (LOCAL booleanValue)? INFILE filePath=STRING_LITERAL INTO TABLE tableName=multipartIdentifier (
        PARTITION partition=identifierList
    )? (COLUMNS TERMINATED BY comma=STRING_LITERAL)? (
        LINES TERMINATED BY separator=STRING_LITERAL
...
queryTerm
    ...
    | left=queryTerm operator=(UNION | EXCEPT | MINUS | INTERSECT) setQuantifier? right=queryTerm # setOperation
    ;
...
querySpecification
    : selectClause intoClause? fromClause? whereClause? aggClause? havingClause? {doris_legacy_SQL_syntax}? queryOrganization #
...
relationPrimary
    : multipartIdentifier optScanParams? materializedViewName? specifiedPartition? tabletList? tableAlias sample? tableSnapshot? relationHint?
...
columnDef
    : colName=identifier type=dataType KEY? (aggType=aggTypeDef)? ((NOT)? nullable=NULL)? (
...
indexDef
    : INDEX indexName=identifier cols=identifierList (
...
inPartitionDef
    : PARTITION (IF NOT EXISTS)? partitionName=identifier (
        VALUES IN (
            (
                LEFT_PAREN constantSeqs+=constantSeq (COMMA constantSeqs+=constantSeq)* RIGHT_PAREN
            )
...
valueExpression
    ...
    | left=valueExpression operator=(PLUS | SUBTRACT | HAT | PIPE | AMPERSAND) right=valueExpression               # arithmeticBinary
